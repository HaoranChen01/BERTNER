...
from transformers import BertPreTrainedModel, AdamW, AlbertPreTrainedModel, AlbertModel
...
class BertSeqClassification(BertPreTrainedModel):
...
class BertBilstmCRF(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # )
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        #     self.crf = CRF(target_size=config.num_labels, average_batch=True, use_cuda=config.use_cuda)
    ...
    def forward(
        ...
        #     [self.dropout(hs) for hs in hidden_states[-3:]], dim=2,
        # )
...
class BertTcnCRF(BertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = self.dropout(sequence_output)
        sequence_output_forward = self.tcn_forward(sequence_output.transpose(1, 2)).transpose(1, 2)
        sequence_output_backward = self.tcn_backward(sequence_output.flip([2]).transpose(1, 2)).transpose(1, 2)
        sequence_output = torch.cat((sequence_output_forward,sequence_output_backward),2)
...
class AlbertBilstmCRF(AlbertPreTrainedModel):
